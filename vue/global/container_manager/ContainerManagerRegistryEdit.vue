...
<template>
  <el-popover :visible="visible" placement="top-end" :width="630" trigger="manual">
    <template #reference>
      <el-button v-if="isAdd" type="primary" plain icon="el-icon-plus" @click="visible = true">{{
        t('add_insecure_registry') }}</el-button>
      <el-button v-else type="primary" plain icon="el-icon-edit" @click="visible = true" style="margin-top: -7px;">{{
        t('edit') }}</el-button>
    </template>
      ...
      <el-form ref="form" :model="insecure_registry_form" @submit.prevent.stop>
        ...
        <el-form-item :label="t('domain_name')" prop="domain_name" :rules="domainNameRules()" label-width="120px">
          <div class="description">
            <el-input v-model.trim="insecure_registry_form.domain_name"
              placeholder="基于上述假设，此处应填写 my-registry.com"></el-input>
            <div v-if="isAdd">
        ...
        <el-form-item :label="t('address')" prop="address" :rules="addressRules" label-width="120px">
          <div class="description">
            <el-input v-model.trim="insecure_registry_form.address"
              placeholder="基于上述假设，此处应填写 http(s)://192.168.30.56:5000"></el-input>
            <div v-if="isAdd">
        ...
        <div v-if="isAdd" class="description">
          <li>
            ...
            <pre
              style="margin-left: 18px; background-color: #333; color: white; padding: 5px 10px; border-radius: 3px;">
          crictl pull my-registry.com/repo-name/image-name:tag</pre>
          </li>
        </div>
        <div style="text-align: right; margin-top: 10px;">
          <el-button icon="el-icon-close" @click="visible = false">{{ t('cancel') }}</el-button>
          <el-button type="primary" icon="el-icon-check" @click="add">{{ t('ok') }}</el-button>
        </div>
      </el-form>
    </div>
  </el-popover>
...
import { reactive, computed, ref, inject, nextTick } from "vue";
...
const insecure_registry_form = reactive({
...
const isAdd = computed(() => props.name == undefined)
const prop = "all.children.target.vars"

function domainNameRules() {
  return [
    { required: true, message: t('domain_name_required'), trigger: 'blur' }
  ]
}
...
function add() {
  form.value.validate(flag => {
    if (flag) {
      ...
      nextTick(() => {
        modelValue.value[insecure_registry_form.domain_name] = insecure_registry_form.address
        visible.value = false
      })
    }
  })
